...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-shell-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-shell-service apps/sample-shell-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-shell-service:1.0-SNAPSHOT
    oc expose svc/sample-shell-service
else

	echo "Launching the application locally..."
	pattern="sample-shell-service"
	files=( $pattern )
	cd ${files[0]}
